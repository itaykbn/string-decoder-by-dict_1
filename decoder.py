    ...
    for char in string:
        ...
        if result is not None:
            ...
            if result != "flip":
                ...
                final_string += reverse(word) + " "
                word = ""
        counter += 1
    return final_string


def decode_char(search_value):
    for key, value in words_dict.items():  # for name, age in dictionary.iteritems():  (for Python 2.x)
        if value == search_value:
            result = key
            return result
    if search_value is not None:
        if represents_int(search_value):
            return "flip"
        else:
            return search_value
